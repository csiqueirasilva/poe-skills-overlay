...
function createTooltipElement(el, width, height, ratio, groupName) {
	...
	div.style.left = el.left.replace(/zoom/gi, 0.5 * ratio);
	div.style.top = el.top.replace(/zoom/gi, 0.5 * ratio);
	...
	$(div).bind('click', (ev) => {
		if(interactingObject === null) {
			...
			if(ev.clientY < parseInt(window.innerHeight / 2)) { // open on bottom side
				objectOptions.style.top = (ev.clientY + (interactingObject.offsetHeight * 1.05)) + 'px' ;
			} else {
				objectOptions.style.top = (ev.clientY - objectOptions.offsetHeight - (interactingObject.offsetHeight * 1.05)) + 'px' ;
			}
